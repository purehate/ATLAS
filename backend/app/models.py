...
class EvidenceItem(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    threat_actor_group_id = Column(
        UUID(as_uuid=True), ForeignKey("threat_actor_groups.id"), nullable=False
    )
    industry_id = Column(UUID(as_uuid=True), ForeignKey("industries.id"), nullable=True)
    technique_id = Column(
        UUID(as_uuid=True), ForeignKey("mitre_techniques.id"), nullable=True
    )
    ...
    __table_args__ = (
        Index(
            "idx_evidence_actor_industry_tech",
...
class ActorIndustryScore(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "threat_actor_group_id", "industry_id", name="uq_actor_industry"
        ),
...
class ActorTechniqueScore(Base):
    ...
    technique_id = Column(
        UUID(as_uuid=True), ForeignKey("mitre_techniques.id"), nullable=False
    )
    industry_id = Column(UUID(as_uuid=True), ForeignKey("industries.id"), nullable=True)
