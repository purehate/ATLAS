...
class CompanyLookupService:
    ...
    async def search_companies(
        ...
        if self.use_opencorporates:
            try:
                opencorp_results = await self._search_opencorporates(query, limit, jurisdiction)
        ...
        if len(results) < limit and not self.use_opencorporates:
            ...
            logger.info("OpenCorporates API key not configured. Set OPENCORPORATES_API_KEY env var for company lookup.")
            return []
    ...
    async def get_company_details(self, company_name: str, jurisdiction: Optional[str] = None) -> Optional[Dict]:
        """
        Get detailed information about a specific company
        """
        # Search first to get company number
        results = await self.search_companies(company_name, limit=1, jurisdiction=jurisdiction)
    ...
    async def _get_opencorporates_details(self, jurisdiction: str, company_number: str) -> Optional[Dict]:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            try:
                ...
                logger.debug(f"Error fetching company details: {e}")
    ...
    async def validate_company_name(self, company_name: str) -> Dict:
        ...
        close_matches = [r for r in results if company_name.lower() in r["name"].lower() or r["name"].lower() in company_name.lower()]
