...
class CalculatorService:
    ...
    async def calculate(
        self, business_vertical: str, sub_vertical: Optional[str] = None
    ) -> CalculateResponse:
        """
            ...
            if parent_id:
                ...
                result = await self.db.execute(
                    select(func.count(EvidenceItem.id)).where(
                        EvidenceItem.industry_id == matched_industry.id
                    )
                )
                ...
                result = await self.db.execute(
                    select(func.count(EvidenceItem.id)).where(
                        EvidenceItem.industry_id == parent_id
                    )
                )
    ...
    async def _calculate_actor_industry_scores(self, industry_id: UUID):
        ...
        for evidence in evidence_items:
            ...
            if actor_id not in actor_scores:
                actor_scores[actor_id] = {"total_score": 0.0, "count": 0}
            ...
            source_reliability = (
                (evidence.source.reliability_score / 10.0) if evidence.source else 0.5
            )
    ...
    async def _get_top_techniques(
        self, actor_id: UUID, industry_id: UUID, limit: int = 10
    ) -> List[TechniqueScore]:
    ...
    async def _calculate_actor_technique_scores(
        self, actor_id: UUID, industry_id: UUID
    ):
        ...
        for evidence in evidence_items:
            ...
            industry_bonus = (
                settings.industry_match_bonus if is_industry_specific else 1.0
            )
        ...
        for tech_id, score_data in technique_scores.items():
            ...
            if existing:
                ...
                new_score = ActorTechniqueScore(
                    ...
                    industry_id=industry_id
                    if score_data["has_industry_match"]
                    else None,
                    weighted_score=score_data["total_score"],
    ...
    async def _get_explanations(
        self, actor_id: UUID, industry_id: UUID, limit: int = 5
    ) -> List[Explanation]:
    ...
    def _calculate_confidence(
        self, evidence_count: int, explanations: List[Explanation]
    ) -> str:
        ...
        if evidence_count >= 5 and len(explanations) >= 2:
            ...
            today = date.today()
            recent_count = sum(
                1 for exp in explanations if (today - exp.source_date).days <= 180
            )
