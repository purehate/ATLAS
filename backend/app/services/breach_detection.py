...
class BreachDetectionService:
    ...
    async def check_company_breach(
        self, company_name: str, industry_keywords: Optional[str] = None
    ) -> Dict:
        """
        ...
        breach_evidence = await self._search_company_breach_articles(
    ...
    async def _search_company_breach_articles(
        ...
        company_variants = [
            company_normalized,
            company_normalized.replace(" inc", "")
            .replace(" inc.", "")
            .replace(", inc", "")
            .replace(", inc.", ""),
            company_normalized.replace(" corp", "")
            .replace(" corp.", "")
            .replace(", corp", "")
            .replace(", corp.", ""),
            company_normalized.replace(" llc", "")
            .replace(" llc.", "")
            .replace(", llc", "")
            .replace(", llc.", ""),
            company_normalized.replace(" ltd", "")
            .replace(" ltd.", "")
            .replace(", ltd", "")
            .replace(", ltd.", ""),
        ]
        ...
        for item in evidence_items:
            ...
            breach_mentioned = any(
                kw.lower() in combined_text for kw in BREACH_KEYWORDS
            )
                ...
                articles.append(
                    {
                        ...
                        "excerpt": relevant_sentence[:300]
                        if relevant_sentence
                        else "",  # First 300 chars
                    }
                )
    ...
    async def _check_cisa_kev_company(self, company_name: str) -> List[Dict]:
        ...
        result = await self.db.execute(select(Source).where(Source.name == "CISA KEV"))
        ...
        query = (
            ...
            .options(selectinload(EvidenceItem.source))
            .where(
                and_(EvidenceItem.source_id == kev_source.id, or_(*company_conditions))
            )
        )
    ...
    def _calculate_breach_confidence(self, articles: List[Dict]) -> int:
        ...
        high_confidence_count = sum(
            1 for art in articles if art.get("source") in HIGH_CONFIDENCE_SOURCES
        )
